...
**The contract is the agent's instructions *and* its report card.** The agent writes down what the data product should be (one readable file), and a framework actually *runs* that file against data to check whether it's true.
...
An OLC contract plus a framework answers the harder, data-specific question: **"Does the resulting data product actually satisfy the engineering contract?"** — because it runs the rules against data, rather than just reading them.

## In one sentence

> An AI data agent **proposes** a contract, a framework **proves** it against data (real or synthetic), and the contract **persists** — so you get AI speed with engineering trust, and the data product survives every change of tool, model, and platform.
