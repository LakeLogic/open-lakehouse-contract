...
**Engine:** Snowflake SQL · **Format:** native Snowflake tables · **Storage:** Snowflake database · **Status:** ✅ Live — the full RideFlow mesh runs green on a Snowflake trial, including a native Snowflake **Project** (notebooks + a Task DAG).
...
- **Identifier casing:** Snowflake upper-cases unquoted identifiers, so the runtime uses UPPERCASE casing and a matching dedup `sort_by` ordering to keep keys stable.
- **Type casts:** `TRY_CAST → TO_VARCHAR` and related casts are adapted for Snowflake SQL (part of the `snowflake-engine-fixes` on the reference runtime).
